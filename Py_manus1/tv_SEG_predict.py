...
parser.add_argument('--input', default="Image/sheets/201907-202505w/", type=str, help='folder directory containing test images')
...
parser.add_argument('--param_set', default="set1", type=str, help='indicate place to save prediction results, e.g. Image/sheets/201907-202505w/SEG_sS_set1')
...
parser.add_argument('--box_size_threshold_L', default=1000, type=float, help='minimum box size threshold. 1000 for pair of strips. Default: %(default)s')
parser.add_argument('--box_size_threshold_U', default=1000, type=float, help='maximum box size threshold. 1000 for pair of strips. Default: %(default)s')
parser.add_argument('--rpn_nms_threshold', default=0.7, type=float, help='NMS threshold. Default: %(default)s')
parser.add_argument('--mask_iou_supp_threshold', default=0.5, type=float, help='mask IoU suppression threshold. Default: %(default)s')
...
for idx, sample in enumerate(test_ds):
    ...
    keep_indices = filter_overlapping_masks(result1['masks'] > args.mask_threshold,
                                            result1['scores'],
                                            iou_threshold=args.mask_iou_supp_threshold)
    ...
    pts_ls=[]
    for idx2, ind_mask_map in enumerate(result_masks):
        if result_scores[idx2] < args.box_score_threshold:
            continue

        box = result_boxes[idx2]

        size = abs((box[0] - box[2]) * (box[1] - box[3]))
        asp_ratio = abs((box[1]-box[3]) / (box[0]-box[2]))

        if args.box_size_threshold_L < size < args.box_size_threshold_U and 6 <= asp_ratio <= 30:
            # if idx==1: print(size, asp_ratio)

            # pixels inside a mask is assigned 255-idx2
            binary_mask = ind_mask_map[0,:,:] > args.mask_threshold
            # if two masks are overlapped, remove the overlapped 
            # binary_mask = remove_overlapping_pixels(binary_mask, previous_masks) 
            masks[np.where(binary_mask)] = 255-idx2
    ...
    if verbose: print(f"number of instances: {len(pts_ls)}")
