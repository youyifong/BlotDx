...
from Py_pre_post_processing.crop_strips import crop_strips
from Py_common.tv_utils import random_rotate_and_resize_strip, normalize_img
...
class TrainDataset_strips(Dataset): # noqa
    def __init__(self, img_dir, label_file, HSV, diagnostic_type, num_classes, mask_dir=None, nchan=None, 
        data_augmentation=True, data_aug_ctrl=None, reorder=True,
        normalize=True): # noqa
        """
        ...
        Parameters:
            ...
            HSV: 1 or 2 for HSV-1 or HSV-2, both for both
        ...
        if mask_dir is None:
            ...
            for img_file in img_files:
                ...
                if img.ndim == 2: # if img is grayscale,
                    img = img[np.newaxis, :, :]  # Result is (1, H, W)
                if normalize:
            ...
            # this is not maintained, there may be bugs, e.g. related to 6-channel tiff files
        ...
        if num_classes==4:
            # ignore strips that are not classified as POSITIVE or negative
            self.strips = [all_strips[i] for i in range(len(all_labels)) if
                           all_labels[i] in ['neg_neg', 'neg_POS', 'POS_neg', 'POS_POS']]  # noqa
        ...
        # throw an error if the number of classes is not 2 or 4
            raise Exception(f"Error: num_classes must be 2 or 4 or 7, but got {num_classes}")
    ...
    def __getitem__(self, idx):
        ...
        if self.data_augmentation:
            ...
            img = torch.as_tensor(random_rotate_and_resize_strip(X=[img],
                                                                 ...
                                                                 sharpening=self.data_aug_ctrl[1]), dtype=torch.float32)
        else:
...
class ValDataset_strips(TrainDataset_strips): # noqa
    def __init__(self, img_dir, label_file, HSV, diagnostic_type, num_classes, mask_dir, nchan, normalize): # noqa
...
# this class is actually not used. it has at least one bug, e.g. it does not process tiff files
