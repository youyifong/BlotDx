...
from TV.tv_Dataset_strips import TrainDataset_strips, ValDataset_strips
from TV.CBAM import  ResNet50_CBAM, predict, find_item, CAM_VIS, show_cam_on_image,tensor_to_rgb_image, plot_cam_heatmap_V2
from TV.CBAM import LabelProcessor, get_prediction_df, compute_performance
from TV.CBAM import get_6chan_image
...
parser.add_argument('--model_name',  default='CLS_HSV1_Final_2classes_SEG_sS1_strips_v4_nopretrained_seed0.pth', type=str, help='model name')
...
gpu_id = int(re.search(r'seed(\d+)', args.model_name).group(1))
...
group_names = ['HSV1NEG_HSV2POS', 'HSV1POS_HSV2NEG', 'HSV1NEG_HSV2NEG','HSV1POS_HSV2POS']
...
for grp in group_names:
    ...
    for strip_id in selected_ids:
        ...
        ############################################################################################################
        vis_methods_list = ['CAM','GradCAM', 'GradCAM++', 'Guided GradCAM']
        cmap = 'RdYlBu_r'
        vis_list = []
        for vis_method in vis_methods_list:
            ...
            if vis_method != 'Guided GradCAM':
                vis = show_cam_on_image(rgb_img, heatmap, cmap = cmap, use_rgb=False, use_custom_cmap = True, image_weight = 0.5, cam_method = vis_method)
            else: #different img for guided gradcam
                if nchan == 3:
                    vis = deprocess_image(heatmap)
                elif nchan == 6:
                    vis1 = deprocess_image(heatmap[:,:,:3])
                    vis2 = deprocess_image(heatmap[:,:,3:])
                    vis = get_6chan_image(vis1, vis2)
        ...
        plot_cam_heatmap_V2(plot_name_list, plot_vis_list, sp_id, label,cur_pred_class, cur_pred_prob, final_path, cmap)
