...
show_code=F
library(kyotil)
# library(captioner); fig_nums <- captioner::captioner(prefix = "**Figure**") # removed from CRAN
```

```{r, data, echo=show_code}
# read in the data from a csv file with column names but no row names
# scores1 <- read.csv("../Feature_Heatmap/CLS_HSV1_Final_2classes_SEG_sS1_strips_v4_pretrained_LucasSeed0.pth/score_by_feature_testimage.csv", header = TRUE)
scores1 <- read.csv("../Feature_Heatmap/CLS_HSV1_Final_2classes_SEG_sS1_strips_v4_pretrained_seed0.pth/score_by_feature_testimage.csv", header = TRUE)
scores1_1 <- read.csv("../Feature_Heatmap/CLS_HSV1_Final_2classes_SEG_sS1_strips_v4_pretrained_seed1.pth/score_by_feature_testimage.csv", header = TRUE)
scores1_2 <- read.csv("../Feature_Heatmap/CLS_HSV1_Final_2classes_SEG_sS1_strips_v4_pretrained_seed2.pth/score_by_feature_testimage.csv", header = TRUE)

# scores2 <- read.csv("../Feature_Heatmap/CLS_HSV2_Final_2classes_SEG_sS1_strips_v4_pretrained_LucasSeed0.pth/score_by_feature_testimage.csv", header = TRUE)
scores2 <- read.csv("../Feature_Heatmap/CLS_HSV2_Final_2classes_SEG_sS1_strips_v4_pretrained_seed0.pth/score_by_feature_testimage.csv", header = TRUE)
scores2_1 <- read.csv("../Feature_Heatmap/CLS_HSV2_Final_2classes_SEG_sS1_strips_v4_pretrained_seed1.pth/score_by_feature_testimage.csv", header = TRUE)
scores2_2 <- read.csv("../Feature_Heatmap/CLS_HSV2_Final_2classes_SEG_sS1_strips_v4_pretrained_seed2.pth/score_by_feature_testimage.csv", header = TRUE)

scores1_nopre   <- read.csv("../Feature_Heatmap/CLS_HSV1_Final_2classes_SEG_sS1_strips_v4_nopretrained_seed0.pth/score_by_feature_testimage.csv", header = TRUE)
scores1_1_nopre <- read.csv("../Feature_Heatmap/CLS_HSV1_Final_2classes_SEG_sS1_strips_v4_nopretrained_seed1.pth/score_by_feature_testimage.csv", header = TRUE)
scores1_2_nopre <- read.csv("../Feature_Heatmap/CLS_HSV1_Final_2classes_SEG_sS1_strips_v4_nopretrained_seed2.pth/score_by_feature_testimage.csv", header = TRUE)

scores2_nopre   <- read.csv("../Feature_Heatmap/CLS_HSV2_Final_2classes_SEG_sS1_strips_v4_nopretrained_seed0.pth/score_by_feature_testimage.csv", header = TRUE)

# feature_importance1 contains the mean score (score = sum of activation * gradient across spatial dimensions) for each of the `r nrow(scores1)` features in the final CNN layer before the Global Average Pooling layer (SEG_sS1_strips_v4 input). The mean is taken across `r ncol(scores1)` test images. feature_importance2 is defined similarly.


order(abs(scores1[,"X2016.09.01_CZ_01_209"]), decreasing = TRUE)[1:10] - 1
tmp = apply(scores1, 2, function(x) order(abs(x), decreasing = TRUE)[1:10])
top10f = as.integer(names(sort(table(tmp), de=T)[1:10]))-1; top10f



feature_importance1 <- rowMeans(abs(scores1))
feature_importance1_1 <- rowMeans(abs(scores1_1))
feature_importance1_2 <- rowMeans(abs(scores1_2))

feature_importance2 <- rowMeans(abs(scores2))
feature_importance2_1 <- rowMeans(abs(scores2_1))
feature_importance2_2 <- rowMeans(abs(scores2_2))

feature_importance1_nopre <- rowMeans(abs(scores1_nopre))
feature_importance1_1_nopre <- rowMeans(abs(scores1_1_nopre))
feature_importance1_2_nopre <- rowMeans(abs(scores1_2_nopre))

feature_importance2_nopre <- rowMeans(abs(scores2_nopre))
...
# find the index of the top k features in feature_importance1
top_feature1_index <- order(feature_importance1, decreasing = T)[1:k]
...
# find the index of the top 4 features in feature_importance2
top_feature2_index <- order(feature_importance2, decreasing = T)[1:k]
top_feature2_1_index <- order(feature_importance2_1, decreasing = T)[1:k]
top_feature2_2_index <- order(feature_importance2_2, decreasing = T)[1:k]

top_feature1_nopre_index <- order(feature_importance1_nopre, decreasing = T)[1:k]
top_feature1_1_nopre_index <- order(feature_importance1_1_nopre, decreasing = T)[1:k]
top_feature1_2_nopre_index <- order(feature_importance1_2_nopre, decreasing = T)[1:k]

top_feature2_nopre_index <- order(feature_importance2_nopre, decreasing = T)[1:k]

# remove X from the colnames
colnames(scores1) <- gsub("X", "", colnames(scores1))
colnames(scores1_1) <- gsub("X", "", colnames(scores1_1))
colnames(scores1_2) <- gsub("X", "", colnames(scores1_2))

colnames(scores2) <- gsub("X", "", colnames(scores2))
colnames(scores2_1) <- gsub("X", "", colnames(scores2_1))
colnames(scores2_2) <- gsub("X", "", colnames(scores2_2))

colnames(scores1_nopre) <- gsub("X", "", colnames(scores1_nopre))
colnames(scores2_nopre) <- gsub("X", "", colnames(scores2_nopre))

# read gt label
gt_labels <- read.csv("..\\Image\\sS_labels.csv", header = TRUE)

# retrieve FinalHSV1 from gt_labels by matching strip_id column with colnames(scores1)
hsv1_status <- gt_labels[match(colnames(scores1), gt_labels$strip_id), "FinalHSV1"]
mytable(hsv1_status)
# code POSITIVE as 1 and negative as 0
hsv1_status <- ifelse(hsv1_status=="POSITIVE", 1, 0)
# retrieve FinalHSV2 from gt_labels by matching strip_id column with colnames(scores2)
hsv2_status <- gt_labels[match(colnames(scores2), gt_labels$strip_id), "FinalHSV2"]
mytable(hsv2_status)
# code POSITIVE as 1 and negative as 0
hsv2_status <- ifelse(hsv2_status=="POSITIVE", 1, 0)

```

## Distribution of feature mean scores

Score means activiation, or weight x feature. 
...
par(mfrow=c(2,3), oma=c(1,2,0,0))
shrink_margin()

# compute feature mean score, without abs
tmp <-cbind(rowMeans(scores1), rowMeans(scores1_1), rowMeans(scores1_2))
for(i in 1:3) {
  hist(tmp[,i], xlab = "", col = "lightblue", main=""); abline(v=0, col="red")
  title(main = "Seed "%.%i, line=1.2)
}

tmp <-cbind(rowMeans(scores2), rowMeans(scores2_1), rowMeans(scores2_2))
for(i in 1:3) {
  hist(tmp[,i], main = "", xlab = "mean feature score across images", col = "lightblue"); abline(v=0, col="red")
}
...
breaks=c( seq(floor(10*min(tmp))/10, max(tmp), by=0.1), max(tmp))
for(i in 1:3) {
  hist(tmp[,i], main = "", xlab = "mean feature score across images", col = "lightblue", xlim=xlim, breaks = breaks); abline(v=0, col="red")
}
...
par(mfrow=c(4,5), oma=c(1,2,0,0))
shrink_margin()
tmp1=order(feature_importance1, decreasing = T)
tmp2=order(feature_importance2, decreasing = T)
xlim=c(-20,50)
for(i in 1:10) {hist(unlist(scores1[tmp1[i],]), main = "f"%.%(tmp1[i]-1), xlab = "", col = "lightblue", xlim=xlim, freq=F); abline(v=0, col="red", lty=2)}
for(i in 1:10) {hist(unlist(scores2[tmp2[i],]), main = "f"%.%(tmp2[i]-1), xlab = "", col = "lightblue", xlim=xlim, freq=F); abline(v=0, col="red", lty=2)}
...
shrink_margin()
xlim=c(-1,1)
for(i in 1:5) {hist(unlist(scores1[i,]), main = "f"%.%(i-1), xlab = "",  col = "lightblue", xlim=xlim); abline(v=0, col="red", lty=2)}
for(i in 1:5) {hist(unlist(scores2[i,]), main = "f"%.%(i-1), xlab = "feature score", col = "lightblue", xlim=xlim); abline(v=0, col="red", lty=2)}
add.mtext.label.2(c("HSV1","HSV2"), 2, line=0.5, cex=.8, font=2)
```

<br><br>

## Top features for models trained starting from pretrained weights

Top 10 feature scores and their indices (0-based) from three seeds for HSV1:
```{r, echo=show_code}
print(rbind(sort(feature_importance1, decreasing = T)[1:10],
...
rbind(
  top_feature1_index-1,
  top_feature1_1_index-1,
  top_feature1_2_index-1)
cat("Indices that appear in all three sets: ", concatList(
intersect(intersect(top_feature1_index-1, top_feature1_1_index-1), top_feature1_2_index-1), ", "))
cat("Indices that appear in at least two out of three sets: ", concatList(
union(
union(
  intersect(top_feature1_index-1, top_feature1_1_index-1),
  intersect(top_feature1_index-1, top_feature1_2_index-1)),
...
rbind(
  top_feature2_index-1,
  top_feature2_1_index-1,
  top_feature2_2_index-1)
tmp=intersect(intersect(top_feature2_index-1, top_feature2_1_index-1), top_feature2_2_index-1)
if (length(tmp)==0) {
...
union(
union(
  intersect(top_feature2_index-1, top_feature2_1_index-1),
  intersect(top_feature2_index-1, top_feature2_2_index-1)),
...
ylim=c(-20,40)
mymatplot(scores1[,1:n], type = "l", col =1:n, make.legend = F, xlab = "", ylab = "Score", main = "HSV1", draw.x.axis = F, xaxt="n", ylim=ylim)
# put a star at x=top_feature1_index and y=22
points(top_feature1_index, rep(25, length(top_feature1_index)), pch=8, col="red")
par(mar = c(3, 3, 1, 1))
mymatplot(scores2[,1:n], type = "l", col =1:n, make.legend = F, xlab = "Feature", ylab = "Score", main = "HSV2", ylim=ylim)
# put a star at x=top_feature2_index and y=5
points(top_feature2_index, rep(25, length(top_feature2_index)), pch=8, col="red")
```
...
shrink_margin()
tmp = cbind(
            sort(rowMeans(abs(scores1)), decreasing = T), 
            sort(rowMeans(abs(scores1_1)), decreasing = T),
            sort(rowMeans(abs(scores1_2)), decreasing = T),
            sort(rowMeans(abs(scores2)), decreasing = T), 
            sort(rowMeans(abs(scores2_1)), decreasing = T),
            sort(rowMeans(abs(scores2_2)), decreasing = T)
            )
mymatplot(tmp, type = "l", xlab = "Ranked feature", ylab = "Score", main = "", ylim=c(0,10), xlim=c(1,40), col=c(1,1,1,2,2,2), make.legend = F, pch=1, lty=c(1,2,3,1,2,3))
...
# plot(sort(feature_importance1, decreasing = T), type = "b", xlab = "Ranked feature", ylab = "Score", main = "", ylim=c(0,4), xlim=c(1,40)); 
# lines(sort(feature_importance2, decreasing = T), type = "b", col=2)

# investigate how often top_feature1_index are present in the top m features of a test image
m=k
top_feature1_index_in_top_k <- rep(0, ncol(scores1))
for (i in 1:ncol(scores1)) {
  top_feature1_index_in_top_k[i] <- sum(top_feature1_index %in% order(abs(scores1[,i]), decreasing = T)[1:m])
}
# same for top_feature2_index
top_feature2_index_in_top_k <- rep(0, ncol(scores2))
for (i in 1:ncol(scores2)) {
  top_feature2_index_in_top_k[i] <- sum(top_feature2_index %in% order(abs(scores2[,i]), decreasing = T)[1:m])
}
# print the number of times each feature in top_feature1_index is in the top 20 features of a test image
# print(top_feature1_index_in_top_k)
i1 = round(mean(top_feature1_index_in_top_k))
# print the number of times each feature in top_feature2_index is in the top 20 features of a test image
# print(top_feature2_index_in_top_k)
i2 = round(mean(top_feature2_index_in_top_k))
```
...
for (i in 1:p) {
  ...
  auc1[i] <- fastauc(unlist(scores1[i,]), hsv1_status)
...
i=1953+1; j=1846+1
plot(unlist(scores1[i,]), unlist(scores1[j,]), xlab="f1953", ylab="f1846", main="HSV1", col=hsv1_status+1)
abline(v=0, lty=2)
abline(h=0, lty=2)
